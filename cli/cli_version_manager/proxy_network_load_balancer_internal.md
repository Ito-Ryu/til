...
*   **プロトコル:** TCP, SSL, HTTP, HTTPS, HTTP/2, gRPC をサポート。
*   **パススルー型での注意点:** パススルー型 (L4) で UDP などを扱う場合でも、ヘルスチェック自体は TCP ベースで行われるのが一般的です。
*   **ファイアウォール:** Google Cloud のヘルスチェックプローバー (`35.191.0.0/16`, `130.211.0.0/22`) からの通信をバックエンド VM で許可する必要があります。
